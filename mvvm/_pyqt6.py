...
class ObservableCollection(QtCore.QAbstractListModel, _qt_common.ObservableCollectionBase):
    _MODELDATA_ROLE = QtCore.Qt.ItemDataRole.UserRole + 1
...
Command = QtCore.pyqtSlot
Event = QtCore.pyqtSignal
Property = _qt_common.Property

_qt_common.add_event_mixins(QtCore.pyqtBoundSignal)
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for key in tuple(attrs.keys()):
            ...
            if isinstance(value, Property):
                if value.constant:
                    attrs[key] = QtCore.pyqtProperty(value.type, fget=value.fget, fset=value.fset, constant=True)
                else:
                    signal = QtCore.pyqtSignal()
                    signal_name = f"_{key}_property_signal"
                    attrs[signal_name] = signal
                    attrs[key] = QtCore.pyqtProperty(value.type, fget=value.fget, fset=value.fset, notify=signal)
...
class ModelBase(QtCore.QObject, metaclass=ModelMeta):
    ...
    def _propagate_property_changed(self, source: ModelBase, source_property_name, target_property_name=None):
